import topik1 from "./LessonDataTopik/topik1.json";
import topik2 from "./LessonDataTopik/topik2.json";
import topik3 from "./LessonDataTopik/topik3.json";
import topik4 from "./LessonDataTopik/topik4.json";
import topik5 from "./LessonDataTopik/topik5.json";
import topik6 from "./LessonDataTopik/topik6.json";
import topik7 from "./LessonDataTopik/topik7.json";
import topik8 from "./LessonDataTopik/topik8.json";
import topik9 from "./LessonDataTopik/topik9.json";
import topik10 from "./LessonDataTopik/topik10.json";
import topik11 from "./LessonDataTopik/topik11.json";
import topik12 from "./LessonDataTopik/topik12.json";
import topik13 from "./LessonDataTopik/topik13.json";
import topik14 from "./LessonDataTopik/topik14.json";
import topik15 from "./LessonDataTopik/topik15.json";
import topik16 from "./LessonDataTopik/topik16.json";
import topik17 from "./LessonDataTopik/topik17.json";

export const topikLessons = [
  { id: 1, title: "meine Familie - О себе и семье", data: topik1 },
  { id: 2, title: "Mein Arbeitstag - Мой рабочий день", data: topik2 },
  { id: 3, title: "Mein Studium - Моя учеба", data: topik3 },
  {
    id: 4,
    title:
      "Warum ich nach Deutschland gehen möchte - Почему я хочу поехать в Германию?",
    data: topik4,
  },
  { id: 5, title: "Meine Geschwister - Мои братья и сестры", data: topik5 },
  { id: 6, title: "In meiner Freizeit - В свободное время", data: topik6 },
  {
    id: 7,
    title: "Deutschkurs bei Iwex - Курсы немецкого языка в Iwex",
    data: topik7,
  },
  { id: 8, title: "Mein bester Freund - Мой лучший друг", data: topik8 },
  {
    id: 9,
    title:
      "Mein gewünschter Job in Deutschland - Моя желаемая работа в Германии",
    data: topik9,
  },

  { id: 10, title: "Mein Lieblingsessen - Моя любимая еда", data: topik10 },
  { id: 11, title: "Meine Großeltern - Мои бабушка и дедушка", data: topik11 },
  {
    id: 12,
    title: "Sprachenlernen macht Spaß - Изучение языков приносит удовольствие",
    data: topik12,
  },
  {
    id: 13,
    title: "Meine Wohnung - Моя квартира",
    data: topik13,
  },
  {
    id: 14,
    title: "Pläne für die Zukunft - Мои планы на будущее",
    data: topik14,
  },
  {
    id: 15,
    title: "Mein Praktikum im Ausland - Моя практика за границей",
    data: topik15,
  },
  {
    id: 16,
    title: "Mein letzter Geburtstag - Мой последний день рождения",
    data: topik16,
  },
  {
    id: 17,
    title: "Schulzeit - Время в школе",
    data: topik17,
  },
];
